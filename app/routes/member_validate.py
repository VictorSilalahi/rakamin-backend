...
class MemberValidateRoute(Resource):
    ...
    def post(self):
        ...
        member = (
            ...
            .filter_by(email=email, member_type="Reguler")
            .first()
        )
        if not member:
        ...
        if check_password_hash(member.password, pwd):
            ...
            member_token = create_access_token(
                identity=member.member_id, fresh=True, expires_delta=token_range
            )
            member_refresh_token = create_refresh_token(
                identity=member.member_id, expires_delta=token_refresh_range
            )
