...
class RoomsAdminRoute(Resource):
    @jwt_required()
    def get(self):
        rooms_and_members = db.session.execute(
            text(
                "select troom.room_id, troom.room_name, count(troommember.*) as member_count from troom left join troommember ON troom.room_id = troommember.room_id group by troom.room_id"
            )
        )
    ...
    def delete(self):
        ...
        return {
            "msg": "ok",
            "data": "room with id={0} has been deleted!".format(room_id),
        }, 200
