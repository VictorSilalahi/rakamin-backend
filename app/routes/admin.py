...
class AdminRoute(Resource):
    ...
    def post(self):
        ...
        admin = (
            db.session.query(Member).filter_by(email=email, member_type="admin").first()
        )
        if not admin:
        ...
        if self.check_admin_password(admin.password, pwd):
            ...
            admin_token = create_access_token(
                identity=admin.member_id, fresh=True, expires_delta=token_range
            )
            admin_refresh_token = create_refresh_token(
                identity=admin.member_id, expires_delta=token_refresh_range
            )
